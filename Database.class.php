...
function check($val,$search){
	$resultado = strpos($val, $search);
	if($resultado !== FALSE)
		return true;
	else
		return false;
...
function insert_vote($user,$place,$vote,$comment){
	...
	$userSQL= "SELECT COUNT(iduser) AS Total from votes WHERE iduser='".$user."' AND idplace=".$place;
		
	$rows = $this->fetch_all_array($userSQL);
