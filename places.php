<?PHP
	...
	$sql	= "SELECT idplace, p.idtype, s.name state, t.name type, p.name place, description, email, phone, website, extra, schedule, latitude, longitude, CONCAT('/images/places/',p.idplace, '.jpg') as image, vote FROM places p, states s, types t WHERE p.idstate=s.idstate AND p.idtype=t.idtype";
	if ($_GET)
	{
		if (@$_GET['idplace'])
		{
